...
分層 cadence（Patch 19A，省 paddle）：estimate_shift 前移→無效幀直接跳過（不跑 paddle）；其餘幀分兩層——
- **heavy**（paddle 全 `_run_single_frame`，更新 cached y_band + 提供整張 frame mask）：僅 bootstrap / 峰觸發（piggyback brightness_way crest/trough x）/ 距上次 heavy ≥ `realtime_seg_refresh_max_n`
- **light**（多數幀）：沿用 cached y_band 只跑 `extract_motion_curve`

mask 採 **refresh-recent**：heavy 幀整張 frame mask 右對齊覆蓋 buffer 最右 ~frame_width 欄（補回 peak 落後偵測缺口）；light 幀 carry-forward。
...
| 2026-05-29 | — | §8 REALTIME 加「分層 cadence」段（estimate_shift 前移 / heavy-light / refresh-recent mask）| Patch 19A：分層 cadence + refresh-recent |
