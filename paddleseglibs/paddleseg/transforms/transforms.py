...
from paddleseglibs.paddleseg.cvlibs import manager
from paddleseglibs.paddleseg.transforms import functional
...
@manager.TRANSFORMS.add_component
class Denoise:
    def __init__(self, params=[5, 5, 7, 21]):
        self.params = params

    def __call__(self, im, label=None):
        im = functional.denoise(im, self.params[0], self.params[1], self.params[2], self.params[3])
        # if label is not None:
            # label = functional.denoise(label)
        if label is None:
            return (im,)
        else:
            return (im, label)
